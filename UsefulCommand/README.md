...
### \# SageMaker Notebook Instance's Terminal Setting
- 간단하게 터미널의 리뉵스 프로프트 길이 제한 하기 (폴더를 따라 들어가면 너무 길어져서 내가 쓰고 있는 명령어가 주 바뀜이 되어서 잘 보여서)
    ```
    export PS1="[\W]\$ " # --> [~]$  
    export PS1="[\u@\h \W]\$ " # --> [ec2-user@ip-172-16-48-49 ~]$
    ```
- ~/.bashrc 파일에 다음과 같이 넣으면 , 모든 세션에서 적용 됨
    ```
...
- 이후에 아래와 같이 적용
    ```
    vi ~/.bashrc 
    [ec2-user@ip-172-16-48-49 ~]$ source ~/.bashrc 
    ```
